...
<div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4">
        <form method="GET" action="{{ route('guru.jadwal_ujian.index') }}" class="flex gap-4 flex-wrap">
...
<script>
    ...
    function startPretest(bankSoalId) {
        ...
        const url = `/guru/jadwal_ujian/start/${bankSoalId}`;
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                if (data.redirect_url) {
                    ...
                    window.location.href = '{{ route("guru.jadwal_ujian.pretest") }}';
                }
            } else {
    ...
    function startPosttest(bankSoalId) {
        ...
        // Initialize posttest
        fetch('{{ route("guru.jadwal_ujian.storePosttest") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': csrfToken,
        ...
        .then(data => {
            if (data.success) {
                ...
                return fetch('{{ route("guru.jadwal_ujian.startPosttest") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': csrfToken,
