...
<div class="space-y-6">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                @if($participant->pretestHasil)
                                    <a href="{{ route('guru.jadwal_ujian.pretest.results', $session->id) }}#participant-{{ $participant->nisn }}" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300">
                                        Lihat Hasil
                                    </a>
                                @else
    ...
    <div class="flex justify-end gap-3">
        ...
        @if($session->status == 'running')
            <a href="{{ route('guru.jadwal_ujian.pretest.live', $session->id) }}" class="bg-blue-500 hover:bg-blue-600 text-white py-2 px-6 rounded-lg font-medium transition">
                <i class="ri-eye-line mr-2"></i> Monitor Pretest
            </a>
        @endif
        
        @if($session->status == 'finished')
            <a href="{{ route('guru.jadwal_ujian.pretest.results', $session->id) }}" class="bg-purple-500 hover:bg-purple-600 text-white py-2 px-6 rounded-lg font-medium transition">
            ...
            .then(data => {
                if (data.success) {
                    if (data.redirect_url) {
                        ...
                        window.location.href = '{{ route("guru.jadwal_ujian.pretest.live", ":sessionId") }}'.replace(':sessionId', sessionId);
                    }
                } else {
