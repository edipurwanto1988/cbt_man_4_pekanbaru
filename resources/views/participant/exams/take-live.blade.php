...
@section('content')
            ...
            <div class="col-12">
                <div class="mb-4">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">
                        {{ $pretestSession->bankSoal->nama_bank }}
                    </h3>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">Soal <span id="currentStep">1</span>/<span
                            id="totalQuestions">{{ $pretestSession->bankSoal->pertanyaanSoals->count() }}</span></p>
                </div>
                        ...
                        @if($currentQuestion)
                            ...
                            <div class="space-y-3" id="answersContainer">
                                @if(isset($answers['A']) && $answers['A'])
                                    <div class="flex items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer transition-colors answer-option"
                                        data-option="A">
                                        <span
                                            class="w-8 h-8 bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 rounded-full flex items-center justify-center font-bold mr-3">A</span>
                                ...
                                @if(isset($answers['B']) && $answers['B'])
                                    <div class="flex items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer transition-colors answer-option"
                                        data-option="B">
                                        <span
                                            class="w-8 h-8 bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 rounded-full flex items-center justify-center font-bold mr-3">B</span>
                                ...
                                @if(isset($answers['C']) && $answers['C'])
                                    <div class="flex items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer transition-colors answer-option"
                                        data-option="C">
                                        <span
                                            class="w-8 h-8 bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 rounded-full flex items-center justify-center font-bold mr-3">C</span>
                                ...
                                @if(isset($answers['D']) && $answers['D'])
                                    <div class="flex items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer transition-colors answer-option"
                                        data-option="D">
                                        <span
                                            class="w-8 h-8 bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 rounded-full flex items-center justify-center font-bold mr-3">D</span>
                                ...
                                @if(isset($answers['E']) && $answers['E'])
                                    <div class="flex items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer transition-colors answer-option"
                                        data-option="E">
                                        <span
                                            class="w-8 h-8 bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 rounded-full flex items-center justify-center font-bold mr-3">E</span>
                ...
                <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                    <div class="flex items-center">
                        <div
                            class="inline-flex items-center justify-center w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/30 mr-3">
    ...
    option.addEventListener('click', function () {
        if (!hasAnsweredCurrentQuestion) {
            const answerId = this.dataset.option; // ⬅ ambil ID dari array
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        if (data.session_status === 'finished') {
                            ...
                            fetch(`{{ route('participant.exams.calculate', $pretestSession->id) }}`, {
                                method: 'POST', // biasanya calculate itu POST
                                headers: {
                                    'X-CSRF-TOKEN': '{{ csrf_token() }}',
                                    'Accept': 'application/json'
                                }
                                ...
                                .then(response => response.json())
                                .then(result => {
                                    // setelah calculate selesai → redirect ke results
        ...
        <div class="space-y-3" id="answersContainer">
            ${question.answers.map((ans, i) => {
                const option = String.fromCharCode(65 + i); // A, B, C, D
                return `
                    <div 
                        class="flex items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 cursor-pointer transition-colors answer-option"
                        data-option="${ans.id}"
                    >
                        <span class="w-8 h-8 bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-300 rounded-full flex items-center justify-center font-bold mr-3">
                            ${option}
                        </span>
                        <span class="text-gray-900 dark:text-white">${ans.isi_jawaban}</span>
                    </div>
                `;
            }).join("")}
        </div>
