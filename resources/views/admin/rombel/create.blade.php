...
<div class="space-y-6">
    ...
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Tambah Rombel</h1>
            <p class="text-gray-600 dark:text-gray-400">Tambah data rombongan belajar baru</p>
        </div>
    </div>

    <!-- Form -->
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm">
        <form action="{{ route('admin.rombel.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div>
                ...
                <select id="tingkat_id" 
                    ...
                    @foreach($tingkatKelas as $tk)
                        <option value="{{ $tk->id }}" {{ old('tingkat_id') == $tk->id ? 'selected' : '' }}>
                            {{ $tk->nama }}
                        </option>
                    @endforeach
                </select>
