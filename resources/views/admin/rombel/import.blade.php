...
@section('title', 'Import Rombel')

@section('content')
<!-- PageHeading -->
<header class="flex flex-wrap items-center justify-between gap-4 mb-8">
    <div class="flex items-center gap-3">
        <a href="{{ route('admin.rombel.index') }}" class="text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white">
            <i class="ri-arrow-left-line text-xl"></i>
        </a>
        <h1 class="text-gray-900 dark:text-white text-3xl font-bold leading-tight">Import Data Rombel</h1>
    </div>
</header>
                ...
                <div class="ml-3">
                    ...
                    <div class="mt-2 text-sm text-blue-700 dark:text-blue-300">
                        <ul class="list-disc list-inside space-y-1">
                            ...
                            <li>Pastikan file Excel memiliki kolom: tahun_ajaran_id, tingkat_id, kode_kelas, nama_rombel, wali_kelas_id</li>
                            <li>Kode kelas diisi dengan "A", "B", "C", atau "D"</li>
                            <li>Wali kelas bersifat opsional</li>
                            <li>Kombinasi tahun ajaran, tingkat kelas, dan kode kelas harus unik</li>
                        </ul>
                    </div>
                    <div class="mt-4">
                        <form action="{{ route('admin.rombel.template') }}" method="GET" target="_blank" onsubmit="console.log('Download template form submitted'); return true;">
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-800">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Kode Kelas</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Nama Rombel</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Wali Kelas</th>
...
console.log('Rombel import page loaded, script executing');
...
previewBtn.addEventListener('click', async (e) => {
    ...
    console.log('Preview route:', "{{ route('admin.rombel.preview') }}");
    ...
    try {
        ...
        console.log('Sending preview request to:', "{{ route('admin.rombel.preview') }}");

        const response = await fetch("{{ route('admin.rombel.preview') }}", {
            method: 'POST',
            body: formData,
            headers: {
...
function displayPreview(data, errors) {
    ...
    data.forEach((row, index) => {
        ...
        tr.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">${row.tahun_ajaran_id || ''}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">${row.tingkat_id || ''}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">${row.kode_kelas || ''}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">${row.nama_rombel || ''}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">${row.wali_kelas_id || '-'}</td>
...
importBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        const response = await fetch("{{ route('admin.rombel.process') }}", {
            method: 'POST',
            body: formData,
            headers: {
                'X-CSRF-TOKEN': "{{ csrf_token() }}",
            ...
            const progressInterval = setInterval(() => {
                ...
                if (currentProgress >= 100) {
                    ...
                    setTimeout(() => {
                        ...
                        window.location.href = "{{ route('admin.rombel.index') }}";
                    }, 500);
                }
