...
@section('content')
    ...
    <header class="flex flex-wrap items-center justify-between gap-4 mb-8">
        ...
        <a href="{{ route('admin.bank_soals.create') }}" class="flex min-w-[84px] cursor-pointer items-center justify-center gap-2 overflow-hidden rounded-lg h-10 px-4 bg-primary text-white text-sm font-bold leading-normal tracking-wide hover:bg-primary/90 transition-colors">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 mb-6">
        <form method="GET" action="{{ route('admin.bank_soals.index') }}" class="flex gap-4 flex-wrap">
                                ...
                                <div class="flex items-center justify-end gap-2">
                                    ...
                                    <div id="dropdown-{{ $bankSoal->id }}" class="hidden origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 divide-y divide-gray-100 dark:divide-gray-700 focus:outline-none z-[9999]" role="menu" aria-orientation="vertical" aria-labelledby="menu-button-{{ $bankSoal->id }}" tabindex="-1">
                                        <div class="py-1" role="none">
                                            <a href="{{ route('admin.bank_soals.show', $bankSoal->id) }}" class="flex items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700" role="menuitem">
                                            ...
                                            @if($bankSoal->pertanyaanSoals->count() == 0)
                                                <form action="{{ route('admin.bank_soals.destroy', $bankSoal->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus bank soal ini?');">
                                                    @csrf
                                                    @method('DELETE')
