...
@section('content')
    ...
    <div class="overflow-hidden rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/50">
        <form action="{{ route('admin.bank_soals.update', $bankSoal->id) }}" method="POST" class="space-y-6 p-6">
            @csrf
            @method('PUT')
                ...
                <div>
                    ...
                    <select name="created_by" id="created_by" required
                        ...
                        @foreach ($gurus as $guru)
                            <option value="{{ $guru->id_guru }}" {{ old('created_by', $bankSoal->created_by) == $guru->id_guru ? 'selected' : '' }}>
                                {{ $guru->nama_guru }}
                            </option>
                        @endforeach
                    </select>
