...
@section('title', 'Edit Pertanyaan Soal - ' . $bankSoal->nama_bank)

@section('content')
    <!-- PageHeading -->
    <header class="flex flex-wrap items-center justify-between gap-4 mb-8">
        <div>
            <h1 class="text-gray-900 dark:text-white text-3xl font-bold leading-tight">Edit Pertanyaan Soal</h1>
            <p class="text-gray-600 dark:text-gray-400">Bank Soal: {{ $bankSoal->nama_bank }}</p>
        </div>
        <a href="{{ route('admin.pertanyaan_soal.index', $bankSoal->id) }}" class="flex min-w-[84px] cursor-pointer items-center justify-center gap-2 overflow-hidden rounded-lg h-10 px-4 bg-gray-200 text-gray-700 text-sm font-bold leading-normal tracking-wide hover:bg-gray-300 transition-colors">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6">
        <form action="{{ route('admin.pertanyaan_soal.update', [$bankSoal->id, $pertanyaanSoal->id]) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <select name="jenis_soal"
                        ...
                        @if($bankSoal->type_test == 'pretest')
                            <option value="pilihan_ganda" {{ old('jenis_soal', $pertanyaanSoal->jenis_soal) == 'pilihan_ganda' ? 'selected' : '' }}>Pilihan Ganda</option>
                        @else
                            <option value="pilihan_ganda" {{ old('jenis_soal', $pertanyaanSoal->jenis_soal) == 'pilihan_ganda' ? 'selected' : '' }}>Pilihan Ganda</option>
                            <option value="esai" {{ old('jenis_soal', $pertanyaanSoal->jenis_soal) == 'esai' ? 'selected' : '' }}>Esai</option>
                            <option value="benar_salah" {{ old('jenis_soal', $pertanyaanSoal->jenis_soal) == 'benar_salah' ? 'selected' : '' }}>Benar/Salah</option>
                        @endif
                    </select>
                    @error('jenis_soal')
                ...
                <div>
                    ...
                    <div id="pertanyaan-editor">
                        <textarea name="pertanyaan"
                                  ...
                                  rows="4"
                                  class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary dark:bg-gray-700 dark:text-white"
                                  required>{{ old('pertanyaan', $pertanyaanSoal->pertanyaan) }}</textarea>
                    </div>
                    @error('pertanyaan')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    @if($pertanyaanSoal->gambar_soal)
                        <div class="mb-2">
                            <img src="{{ asset('uploads/pertanyaan_soals/' . $pertanyaanSoal->gambar_soal) }}" alt="Gambar Soal" class="h-24 w-24 rounded object-cover">
                    ...
                    <input type="file" 
                           ...
                           class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary dark:bg-gray-700 dark:text-white">
                    <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Format: JPG, PNG, GIF. Maks: 2MB. Kosongkan jika tidak ingin mengubah gambar.</p>
                    @error('gambar_soal')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div id="jawaban-pilihan-ganda" class="{{ $pertanyaanSoal->jenis_soal != 'pilihan_ganda' ? 'hidden' : '' }}">
                    ...
                    <div id="container-jawaban" class="space-y-3">
                        @foreach($pertanyaanSoal->jawabanSoals as $jawaban)
                            <div class="jawaban-item flex items-center gap-3">
                                <div class="flex-shrink-0">
                                    <span class="w-8 h-8 flex items-center justify-center bg-gray-200 dark:bg-gray-700 rounded-full text-sm font-medium">{{ $jawaban->opsi }}</span>
                                </div>
                                <div class="flex-grow">
                                    <input type="text"
                                           name="jawaban[{{ $jawaban->opsi }}]"
                                           value="{{ old('jawaban.' . $jawaban->opsi, $jawaban->isi_jawaban) }}"
                                           placeholder="Isi jawaban {{ $jawaban->opsi }}"
                                           class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary dark:bg-gray-700 dark:text-white">
                                </div>
                                <div class="flex-shrink-0">
                                    @if($jawaban->gambar_jawaban)
                                        <img src="{{ asset('uploads/jawaban_soals/' . $jawaban->gambar_jawaban) }}" alt="Gambar Jawaban" class="h-10 w-10 rounded object-cover mb-1">
                                        <p class="text-xs text-gray-500 dark:text-gray-400">Gambar saat ini</p>
                                    @endif
                                    <input type="file"
                                           name="gambar_jawaban[{{ $jawaban->opsi }}]"
                                           accept="image/jpeg,image/png,image/jpg,image/gif"
                                ...
                                <div class="flex-shrink-0">
                                    <label class="inline-flex items-center">
                                        <input type="radio" name="is_benar" value="{{ $jawaban->opsi }}" {{ $jawaban->is_benar ? 'checked' : '' }} class="form-radio text-primary">
                                        <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Benar</span>
                                    </label>
                                </div>
                                <div class="flex-shrink-0">
                                    <button type="button" class="hapus-jawaban text-red-600 hover:text-red-800 {{ $loop->index <= 1 ? 'hidden' : '' }}">
                ...
                <!-- Jawaban Benar/Salah -->
                <div id="jawaban-benar-salah" class="{{ $pertanyaanSoal->jenis_soal != 'benar_salah' ? 'hidden' : '' }}">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">
                        Jawaban Benar <span class="text-red-500">*</span>
                    </label>
                    <div class="flex gap-6">
                        <label class="inline-flex items-center">
                            <input type="radio" name="jawaban_benar_salah" value="T" {{ $pertanyaanSoal->jawabanSoals->where('opsi', 'T')->where('is_benar', true)->count() > 0 ? 'checked' : '' }} class="form-radio text-primary">
                            <span class="ml-2 text-gray-700 dark:text-gray-300">Benar</span>
                        </label>
                        <label class="inline-flex items-center">
                            <input type="radio" name="jawaban_benar_salah" value="F" {{ $pertanyaanSoal->jawabanSoals->where('opsi', 'F')->where('is_benar', true)->count() > 0 ? 'checked' : '' }} class="form-radio text-primary">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" 
                               name="bobot_benar" 
                               id="bobot_benar" 
                               value="{{ old('bobot_benar', $pertanyaanSoal->bobot_benar) }}"
                               step="0.01"
                               min="0"
                               class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary dark:bg-gray-700 dark:text-white">
                        @error('bobot_benar')
                    ...
                    <div>
                        ...
                        <input type="number" 
                               name="bobot_salah" 
                               id="bobot_salah" 
                               value="{{ old('bobot_salah', $pertanyaanSoal->bobot_salah) }}"
                               step="0.01"
                               class="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary dark:bg-gray-700 dark:text-white">
                        @error('bobot_salah')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="mt-8 flex justify-end gap-3">
                <a href="{{ route('admin.pertanyaan_soal.index', $bankSoal->id) }}"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function updateHapusButtons() {
            ...
            // Minimal 3 opsi (A-C), jadi sembunyikan tombol hapus jika hanya ada 3 opsi
            if (jawabanItems.length <= 3) {
