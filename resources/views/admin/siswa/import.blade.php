...
<header class="flex flex-wrap items-center justify-between gap-4 mb-8">
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-gray-900 dark:text-white text-3xl font-bold leading-tight">Import Data Siswa</h1>
    </div>
</header>
                ...
                <div class="ml-3">
                    ...
                    <div class="mt-2 text-sm text-blue-700 dark:text-blue-300">
                        <ul class="list-disc list-inside space-y-1">
                            ...
                            <li>Pastikan file Excel memiliki kolom: nisn, nama_siswa, jenis_kelamin, email, password</li>
                            <li>Jenis kelamin diisi dengan "L" untuk Laki-laki dan "P" untuk Perempuan</li>
                            <li>Email bersifat opsional, namun password wajib diisi</li>
                            <li>NISN harus unik dan tidak boleh duplikat dengan data yang sudah ada</li>
                        </ul>
                    </div>
                    <div class="mt-4">
                        <form action="{{ route('admin.siswa.template') }}" method="GET" target="_blank" onsubmit="console.log('Download template form submitted'); return true;">
...
console.log('Import page loaded, script executing');
...
previewBtn.addEventListener('click', async (e) => {
    ...
    console.log('Preview route:', "{{ route('admin.siswa.preview') }}");
    ...
    try {
        ...
        console.log('Sending preview request to:', "{{ route('admin.siswa.preview') }}");

        const response = await fetch("{{ route('admin.siswa.preview') }}", {
            method: 'POST',
            body: formData,
            headers: {
...
function displayPreview(data, errors) {
    ...
    data.forEach((row, index) => {
        ...
        tr.innerHTML = `
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">${row.nisn || ''}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">${row.nama_siswa || ''}</td>
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">${row.jenis_kelamin || ''}</td>
...
importBtn.addEventListener('click', async () => {
    ...
    try {
        ...
        const response = await fetch("{{ route('admin.siswa.process') }}", {
            method: 'POST',
            body: formData,
            headers: {
                'X-CSRF-TOKEN': "{{ csrf_token() }}",
